  ...
  return SelectedUser ? (
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
      ...
      <div className="flex items-center gap-3 py-3 mx-4 border-b border-stone-500">
        ...
        {/* Back to list (mobile) */}
        <img onClick={() => setSelectedUser(null)} src={assets.arrow_icon} alt="Back" className="md:hidden w-5" />
        {/* Open right info (mobile) */}
        <button onClick={() => setIsRightOpen(true)} className="md:hidden p-2 -m-2">
          <img src={assets.help_icon} alt="Info" className="w-5" />
        </button>
        {/* Desktop help icon unchanged */}
        <img src={assets.help_icon} alt="Help" className="max-md:hidden max-w-5" />
        {/* Mobile action menu */}
        <div ref={menuRef} className="relative md:hidden">
          <button type="button" aria-haspopup="menu" aria-expanded={isMenuOpen} onClick={() => setIsMenuOpen(v => !v)} className="p-2 -m-2">
            <img src={assets.menu_icon} alt="Menu" className="w-5" />
          </button>
          <div className={`absolute right-0 top-full mt-2 z-50 w-40 rounded-md bg-[#282142] border border-gray-600 text-gray-100 shadow-lg ${isMenuOpen ? 'block' : 'hidden'}`} role="menu">
            <button onClick={() => { setIsMenuOpen(false); navigate('/profile') }} className="block w-full text-left text-sm px-4 py-2 hover:text-white" role="menuitem">Edit Profile</button>
            <hr className="border-gray-600" />
            <button onClick={() => { setIsMenuOpen(false); logout() }} className="block w-full text-left text-sm px-4 py-2 hover:text-white" role="menuitem">Logout</button>
          </div>
        </div>
      ...
      {isRightOpen && (
        <div className="md:hidden fixed inset-0 z-50 bg-[#0b0a13]/95 text-white overflow-y-auto">
          <div className="flex items-center justify-between p-4 border-b border-white/10">
            <h3 className="text-lg">Profile</h3>
            <button onClick={() => setIsRightOpen(false)} className="p-2 -m-2">
              <img src={assets.arrow_icon} alt="Close" className="w-5 rotate-180" />
            </button>
          </div>
          ...
          <div className='px-5 text-xs'>
            ...
            <div className='mt-2 max-h-[300px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80'>
              {messages.filter(m => m.image).map((m, idx) => (
                <div key={idx} onClick={() => window.open(m.image)} className='cursor-pointer rounded'>
                  <img src={m.image} alt="" className='h-full rounded-md' />
                </div>
              ))}
            </div>
          </div>
          <div className='p-5'>
            <button onClick={() => { setIsRightOpen(false); logout() }} className='w-full bg-gradient-to-r from-purple-400 to-violet-600 text-white border-none text-sm font-light py-3 rounded-full'>
              Logout
            </button>
          </div>
        </div>
